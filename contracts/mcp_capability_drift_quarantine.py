...
SHA40=re.compile(r"^[0-9a-f]{40}$"); SHA256=re.compile(r"^sha256:[0-9a-f]{64}$")
...
class MCPCapabilityDriftQuarantine(gl.Contract):
    ...
    def register_server(self, package: str, repository: str, baseline_version: str, manifest_path: str, policy: str, deployment_controller: Address) -> typing.Any:
        ...
        self.servers[sid]=canon({"id":int(sid),"package":pkg,"repository":repo,"baseline_version":version,"manifest_path":path,"policy":policy,"controller":controller,"policy_revision":1,"active":True,"baseline_request":0})
        return sid

    @gl.public.write
    def propose_update(self, server_id: u256, candidate_version: str, action_digest: str, expiry_seconds: u256) -> typing.Any:
        server=self._server(server_id)
        if server is None: return "SERVER_NOT_FOUND"
        if sender()!=self.owner: return "ONLY_OWNER"
        version=candidate_version.strip().lower(); digest=action_digest.strip().lower(); seconds=int(expiry_seconds)
        ...
        self.requests[rid]=canon({"id":int(rid),"server_id":int(server_id),"package":server["package"],"repository":server["repository"],"old_version":server["baseline_version"],"new_version":version,"manifest_path":server["manifest_path"],"policy":server["policy"],"policy_revision":server["policy_revision"],"controller":server["controller"],"action_digest":digest,"expiry":now()+seconds,"revision":1,"status":PENDING,"reason":"NOT_ASSESSED","evidence_digest":"","old_commit":"","new_commit":"","old_manifest_digest":"","new_manifest_digest":"","consumed":False,"consumed_at":0})
        return rid
        ...
        def evaluate():
            try:
                ...
                for version,commit in releases:
                    ...
                    if str(cj.get("sha","")).lower()!=commit or SHA40.fullmatch(tree) is None: return unresolved("COMMIT_IDENTITY_MISMATCH")
                    tres=gl.nondet.web.get("https://api.github.com/repos/"+snapshot["repository"]+"/git/trees/"+tree+"?recursive=1",headers=headers); tbody=tres.body or b""
                    if int(tres.status)!=200 or len(tbody)<2 or len(tbody)>MAX_BYTES: return unresolved("TREE_SOURCE_UNAVAILABLE_OR_OVERSIZED")
                    tj=json.loads(tbody.decode("utf-8")); match=None
                    if str(tj.get("sha","")).lower()!=tree or bool(tj.get("truncated",True)): return unresolved("TREE_INCOMPLETE_OR_MISMATCHED")
                    for item in tj.get("tree",[]):
                        if item.get("path")==snapshot["manifest_path"]: match=item
                    if not isinstance(match,dict) or match.get("type")!="blob" or match.get("mode") not in ("100644","100755"): return unresolved("MANIFEST_NOT_REGULAR_BLOB")
                    raw=gl.nondet.web.get("https://raw.githubusercontent.com/"+snapshot["repository"]+"/"+commit+"/"+snapshot["manifest_path"],headers={"Accept":"application/json","User-Agent":"MCPCapabilityDriftQuarantine/1.0"}); body=raw.body or b""
                    if int(raw.status)!=200 or len(body)<2 or len(body)>MAX_BYTES: return unresolved("MANIFEST_SOURCE_UNAVAILABLE_OR_OVERSIZED")
                    blob=hashlib.sha1(b"blob "+str(len(body)).encode("ascii")+b"\x00"+body).hexdigest()
                    if blob!=str(match.get("sha","")).lower() or len(body)!=int(match.get("size",-1)): return unresolved("BLOB_DIGEST_MISMATCH")
                    ...
                    manifests.append((manifest,"sha256:"+hashlib.sha256(body).hexdigest()))
                old,new=manifests
    ...
    def consume_rollout(self, request_id: u256, expected_revision: u256, action_digest: str) -> str:
        ...
        if not server["active"] or server["policy_revision"]!=r["policy_revision"] or server["baseline_version"]!=r["old_version"]:return "BASELINE_OR_POLICY_STALE"
        r["consumed"]=True;r["consumed_at"]=now();self.requests[request_id]=canon(r)
        server["baseline_version"]=r["new_version"];server["baseline_request"]=r["id"];self.servers[u256(r["server_id"])]=canon(server)
