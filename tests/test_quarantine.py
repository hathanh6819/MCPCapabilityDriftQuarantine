...
def mock_sources(vm,old_body=None,new_body=None,ref_status=200,bad_ref=False,truncated=False,bad_blob=False):
    ...
    for version,commit,tree,body in ((OLD,OLD_COMMIT,OLD_TREE,old_body),(NEW,NEW_COMMIT,NEW_TREE,new_body)):
        vm.mock_web(r"/commits/v"+version.replace(".",r"\.")+r"$",{"status":ref_status,"body":json.dumps({"sha":"f"*39 if bad_ref else commit})})
...
def test_architecture_shape():
    ...
    for token in ("/commits/v","/git/commits/","/git/trees/","raw.githubusercontent.com","hashlib.sha1","hashlib.sha256","strict_eq"):assert token in source
...
@pytest.mark.parametrize("kwargs,reason",[({"ref_status":503},"VERSION_TAG_SOURCE_UNAVAILABLE_OR_OVERSIZED"),({"bad_ref":True},"VERSION_TAG_COMMIT_INVALID"),({"truncated":True},"TREE_INCOMPLETE_OR_MISMATCHED"),({"bad_blob":True},"BLOB_DIGEST_MISMATCH"),({"new_body":"not json"},"SOURCE_OR_MODEL_ERROR")])
...
def test_unresolved_recovery_and_stale_revision(direct_deploy,direct_vm):
    c=deploy(direct_deploy);setup(c);mock_sources(direct_vm,ref_status=503);assert c.assess_update(1,1)=="UNRESOLVED"
