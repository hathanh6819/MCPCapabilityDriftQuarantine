...
def setup(c):
    assert c.register_server(PACKAGE,REPO,OLD,digest(manifest(OLD)),PATH,POLICY,CONTROLLER)==1
    assert c.propose_update(1,NEW,digest(manifest(NEW)),DIGEST,86400)==1
...
def digest(body): return "sha256:"+hashlib.sha256(body.encode()).hexdigest()
def mock_sources(vm,old_body=None,new_body=None,status=200):
    old_body=old_body or manifest(OLD);new_body=new_body or manifest(NEW)
    for version,body in ((OLD,old_body),(NEW,new_body)):
        vm.mock_web(r"raw\.githubusercontent\.com/"+REPO+r"/refs/tags/v"+version.replace(".",r"\.")+r"/"+PATH,{"status":status,"body":body})
...
def test_architecture_shape():
    ...
    for token in ("raw.githubusercontent.com","/refs/tags/v","baseline_manifest_digest","candidate_manifest_digest","hashlib.sha256","strict_eq"):assert token in source
    for token in ("api.github.com","hashlib.sha1","/git/trees/"):assert token not in source
...
def test_invalid_server_does_not_mutate(direct_deploy,field,value):
    c=deploy(direct_deploy);args={"package":PACKAGE,"repository":REPO,"baseline_version":OLD,"baseline_manifest_digest":digest(manifest(OLD)),"manifest_path":PATH,"policy":POLICY,"deployment_controller":CONTROLLER};keys={"repo":"repository","version":"baseline_version","path":"manifest_path","controller":"deployment_controller"};args[keys.get(field,field)]=value
...
def test_only_owner_and_duplicate_package(direct_deploy,direct_vm,direct_bob):
    c=deploy(direct_deploy)
    with direct_vm.prank(direct_bob):assert c.register_server(PACKAGE,REPO,OLD,digest(manifest(OLD)),PATH,POLICY,CONTROLLER)=="ONLY_OWNER"
    assert c.register_server(PACKAGE,REPO,OLD,digest(manifest(OLD)),PATH,POLICY,CONTROLLER)==1
    assert c.register_server(PACKAGE,REPO,OLD,digest(manifest(OLD)),PATH,POLICY,CONTROLLER)=="PACKAGE_ALREADY_REGISTERED"

def test_invalid_locked_digests_do_not_create_state(direct_deploy):
    c=deploy(direct_deploy)
    assert c.register_server(PACKAGE,REPO,OLD,"sha256:bad",PATH,POLICY,CONTROLLER)=="INVALID_BASELINE_DIGEST"
    assert c.get_counts()=={"server_count":0,"request_count":0}
    assert c.register_server(PACKAGE,REPO,OLD,digest(manifest(OLD)),PATH,POLICY,CONTROLLER)==1
    assert c.propose_update(1,NEW,"sha256:bad",DIGEST,86400)=="INVALID_CANDIDATE_DIGEST"
    assert c.get_counts()=={"server_count":1,"request_count":0}

def test_decimal_controller_is_canonical_hex_address(direct_deploy):
    c=deploy(direct_deploy);assert c.register_server(PACKAGE,REPO,OLD,digest(manifest(OLD)),PATH,POLICY,int(CONTROLLER,16))==1
...
def test_hidden_payment_is_quarantined(direct_deploy,direct_vm):
    ...
    candidate=manifest(NEW,tools)
    assert c.register_server(PACKAGE,REPO,OLD,digest(manifest(OLD)),PATH,POLICY,CONTROLLER)==1
    assert c.propose_update(1,NEW,digest(candidate),DIGEST,86400)==1
    mock_sources(direct_vm,new_body=candidate);mock_model(direct_vm,False)
...
def test_swapped_manifest_versions_fail_closed(direct_deploy,direct_vm):
    ...
    assert c.assess_update(1,1)=="UNRESOLVED";assert c.get_request(1)["reason"]=="TAGGED_MANIFEST_DIGEST_MISMATCH"

def test_wrong_locked_manifest_digest_fails_closed(direct_deploy,direct_vm):
    c=deploy(direct_deploy)
    assert c.register_server(PACKAGE,REPO,OLD,"sha256:"+"f"*64,PATH,POLICY,CONTROLLER)==1
    assert c.propose_update(1,NEW,digest(manifest(NEW)),DIGEST,86400)==1
    mock_sources(direct_vm)
    assert c.assess_update(1,1)=="UNRESOLVED"
    assert c.get_request(1)["reason"]=="TAGGED_MANIFEST_DIGEST_MISMATCH"

@pytest.mark.parametrize("kwargs,reason",[({"status":503},"TAGGED_MANIFEST_UNAVAILABLE_OR_OVERSIZED"),({"new_body":"not json"},"TAGGED_MANIFEST_DIGEST_MISMATCH")])
...
def test_unresolved_recovery_and_stale_revision(direct_deploy,direct_vm):
    c=deploy(direct_deploy);setup(c);mock_sources(direct_vm,status=503);assert c.assess_update(1,1)=="UNRESOLVED"
...
def test_parallel_safe_candidate_is_stale_after_first_baseline_promotion(direct_deploy,direct_vm):
    c=deploy(direct_deploy);assert c.register_server(PACKAGE,REPO,OLD,digest(manifest(OLD)),PATH,POLICY,CONTROLLER)==1
    assert c.propose_update(1,NEW,digest(manifest(NEW)),DIGEST,86400)==1;assert c.propose_update(1,NEW,digest(manifest(NEW)),"sha256:"+"c"*64,86400)==2
